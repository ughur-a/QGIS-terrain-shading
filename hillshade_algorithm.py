...
class HillshadeAlgorithm(QgsProcessingAlgorithm):
    ...
    def initAlgorithm(self, config):
        ...
        self.addParameter(
            QgsProcessingParameterBoolean(
                self.BIDIRECTIONAL, self.tr("Bidirectional hillshade"), False, False
            )
        )
        ...
        self.addParameter(
            QgsProcessingParameterEnum(
                self.DENOISE, self.tr("Denoise"), self.DENOISE_TYPES, defaultValue=1
            )
        )

        self.addParameter(
            QgsProcessingParameterBoolean(
                self.BYTE_FORMAT, self.tr("Byte sized output (0-255)"), False, False
            )
        )
    ...
    def processAlgorithm(self, parameters, context, feedback):
        ...
        if (
            bidirectional
        ):  # because of vector addition, the effective lighting is shifted
            direction -= 45
            if direction < 0:
                direction += 360
        ...
        dem.set_output(self.output_model, data_format_override=byte, compression=True)
        ...
        s = np.radians(
            360 - direction
        )  # reverse the sequence (more simple than to fiddle with sin/cos...)
        ...
        for mx_view_in, gdal_take, mx_view_out, gdal_put in window_loop(
            ...
            if smooth == 2:
                mx_z = median_filter(mx_z, 2)

            mx_a[:], mx_a2[:] = 0, 0

            for (y, x), weight in np.ndenumerate(win):
                view_in, view_out = view(
                    y - win_size // 2, x - win_size // 2, mx_z.shape
                )
            ...
            # slope = dz / dx;
            px = 1 / (
                abs(pix_x) * np.sum(win[win > 0])
            )  # take 1/dist to use multiplication
