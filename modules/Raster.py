...
class Raster:
    def __init__(self, qgis_raster_object, crs=None):
        ...
        self.extent = [raster_x_min, raster_y_min, raster_x_max, raster_y_max]
    ...
    def verify_raster(self):
        ...
        if units != 0:
            ...
            if units == 6:
                self.pix_x, self.pix_y = self.deg_to_m(
                    self.pix_x, self.pix_y, (self.extent[3] + self.extent[1]) / 2
                )
    ...
    def add_to_buffer(self, matrix, gdal_put, mode=DUMP, automatic_save=True):
        """
    ...
    def set_output(
        self, file_name, no_data=np.nan, data_format_override=None, compression=True
    ):
        """
        ...
        if data_format_override:
            ...
            self.data_format = (
                INT if ProcessingConfig.getSetting("CONVERT_INT") else FLOAT
            )
    ...
    def write_output(self):
        if isinstance(self.buffer, np.ndarray):  # buffered mode
            ...
            if self.data_format != FLOAT:
                sd, max_val, median = (
                    np.std(self.buffer),
                    np.max(abs(self.buffer)),
                    np.median(self.buffer),
                )
                if max_val > median + 5 * sd:
                    max_val = median + 5 * sd
                self.buffer /= max_val
                if self.data_format == INT:
                    self.buffer *= 32767
                elif self.data_format == BYTE:
                    self.buffer *= 255
    ...
    def deg_to_m(self, diff_x, diff_y, latitude):
        ...
        return (diff_x * 111111 * np.cos(np.radians(latitude)), diff_y * 111111)
